		...
		"twitter")
			...
			if [[ "$PROGRAMA_QUE_DEBERIA_EMITIRSE_EPISODIO" != *"#prueba"* ]] ; then
				if [ ! -z "$PROGRAMA_QUE_DEBERIA_EMITIRSE_TWITTER" ] ; then
					TWEET="Empieza $PROGRAMA_QUE_DEBERIA_EMITIRSE $PROGRAMA_QUE_DEBERIA_EMITIRSE_EPISODIO ($PROGRAMA_QUE_DEBERIA_EMITIRSE_TIPO) @$PROGRAMA_QUE_DEBERIA_EMITIRSE_TWITTER - Escúchalo en http://$WEB_SERVER/"
				else
